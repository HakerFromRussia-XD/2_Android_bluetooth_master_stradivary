...
The original source PNG hashes are recorded below. The 14 packaged illustrations now use 256×256 RGBA WebP in `app/src/main/res/drawable-nodpi`, resized with Lanczos and encoded losslessly after resizing, as selected by the user. Original PNGs remain recoverable from Git history. Every card now has the former Marathon height: a 40 dp header and an artwork slot of `(contentWidth - 10 dp) * 822 / 1024`, followed by the existing counter and progress bar. The artwork slot spans the same left/right edges as the progress bar. Each alpha-cropped illustration fits inside it, bottom-centered without stretching or cutting off content; narrower artwork therefore retains empty space at the sides. The artwork-to-counter and counter-to-progress-bar gaps are both 4 dp.

Image regions are decoded on `Dispatchers.IO`, using power-of-two sampling followed by scaling to the actual slot size in physical pixels. A serialized decoder prevents concurrent allocations and a 16 MiB LRU cache reuses screen-sized bitmaps during scrolling. Evicted images are not recycled while Compose may still display them. Only one resolution is packaged for each illustration; cards and future enlarged dialogs can decode the same file at their target size. Previously `ImageBitmap.imageResource` decoded the full original image synchronously during composition.
...
Three RGBA PNGs in `app/src/main/res/drawable-nodpi/achievement_star_{bronze,silver,gold}.png` were generated with the built-in image_gen tool. Generated masters were 1254×1254 RGBA; packaged stars are Lanczos-downsampled 96×96 RGBA with actual alpha values 0–255. The existing background decoder and 16 MiB cache automatically reduce them to the 18 dp display size. One bronze star, two silver stars or three gold stars are centered above the counter; no stars are shown without bronze. The 18 dp star row overlays the bottom center of the artwork in a Box. It takes no layout space: artwork size, counter position and card height are unchanged. The counter keeps its 4 dp gaps.
...
User selected 256×256 after comparing 256 and 512 at card size and twice that size. The 14 packaged WebP images total 957,178 bytes, down from 21,774,505 bytes of PNG (95.6% smaller). Resolution reduction is intentional; WebP encoding adds no further loss. Alpha-content bounds in the catalog were recalculated from each resized image. The reference Marathon aspect ratio remains unchanged so the card layout does not shift. Stars stay at 96×96; their overlay/fade and tier-based colour rules are unchanged. This change does not add the future enlarged illustration to the information dialog.
