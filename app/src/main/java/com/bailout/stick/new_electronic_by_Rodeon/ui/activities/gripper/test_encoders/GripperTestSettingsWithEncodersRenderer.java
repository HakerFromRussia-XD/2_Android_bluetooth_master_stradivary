...
import com.bailout.stick.new_electronic_by_Rodeon.events.rx.RxUpdateMainEvent;
import com.bailout.stick.new_electronic_by_Rodeon.models.FingerAngle;
import com.bailout.stick.new_electronic_by_Rodeon.presenters.Load3DModelNew;
import com.bailout.stick.new_electronic_by_Rodeon.ui.activities.gripper.common.RawResourceReader;
import com.bailout.stick.new_electronic_by_Rodeon.ui.activities.gripper.common.ShaderHelper;
import com.bailout.stick.new_electronic_by_Rodeon.ui.activities.gripper.common.TextureHelper;
 ...
 * used instead.
 */
public class GripperTestSettingsWithEncodersRenderer implements GLSurfaceView.Renderer{
	/** Used for debug logs. */
	...
	private final com.bailout.stick.new_electronic_by_Rodeon.ui.activities.gripper.test_encoders.ErrorHandler errorHandler;
	 ...
	 */
	public GripperTestSettingsWithEncodersRenderer(final Context fragmentGripperSettings, ErrorHandler errorHandler) {
...
//		}
		...
		if (GripperTestScreenWithEncodersActivity.Companion.getSide() == 0) {
	...
	private void foreFinger (int[] shaderMassiv, int idForSelectObject) {
		...
		if (GripperTestScreenWithEncodersActivity.Companion.getAnimationInProgress4()) {
			Matrix.setIdentityM(currentRotation, 0);
		...
		/** поворот вокруг первой оси */
		if (GripperTestScreenWithEncodersActivity.Companion.getAnimationInProgress4()) {
			angleForeFingerTransfer = GripperTestScreenWithEncodersActivity.Companion.getAngleFinger4();
...
//		GLES20.glUniform1f(codeSelectUniform, (float) idForSelectObject);
	...
	private void middleFinger (int[] shaderMassiv, int idForSelectObject) {
		...
		/** поворот вокруг первой оси */
		if (GripperTestScreenWithEncodersActivity.Companion.getAnimationInProgress3()) {
			Matrix.setIdentityM(currentRotation, 0);
			if (GripperTestScreenWithEncodersActivity.Companion.getSide() == 0) {
		...
		/** поворот вокруг первой оси */
		if (GripperTestScreenWithEncodersActivity.Companion.getAnimationInProgress3()) {
			angleMiddleFingerTransfer = GripperTestScreenWithEncodersActivity.Companion.getAngleFinger3();
	...
	private void ringFinger (int[] shaderMassiv, int idForSelectObject) {
		...
		/** поворот вокруг первой оси */
		if (GripperTestScreenWithEncodersActivity.Companion.getAnimationInProgress2()) {
			Matrix.setIdentityM(currentRotation, 0);
			if (GripperTestScreenWithEncodersActivity.Companion.getSide() == 0) {
		...
		/** поворот вокруг первой оси */
		if (GripperTestScreenWithEncodersActivity.Companion.getAnimationInProgress2()) {
			angleRingFingerTransfer = GripperTestScreenWithEncodersActivity.Companion.getAngleFinger2();
	...
	private void littleFinger (int[] shaderMassiv, int idForSelectObject) {
		...
		/** поворот вокруг первой оси */
		if (GripperTestScreenWithEncodersActivity.Companion.getAnimationInProgress1()) {
		...
		/** поворот вокруг первой оси */
		if (GripperTestScreenWithEncodersActivity.Companion.getAnimationInProgress1()) {
			angleLittleFingerTransfer = GripperTestScreenWithEncodersActivity.Companion.getAngleFinger1();
	...
	private void bigFinger (int[] shaderMassiv, int idForSelectObject)  {
		...
		/** поворот вокруг первой оси */
		if (GripperTestScreenWithEncodersActivity.Companion.getAnimationInProgress5()) {
			angleBigFingerTransfer1 = GripperTestScreenWithEncodersActivity.Companion.getAngleFinger5();
		...
		/** поворот вокруг второй оси */
		if (GripperTestScreenWithEncodersActivity.Companion.getAnimationInProgress6()) {
			angleBigFingerTransfer2 = GripperTestScreenWithEncodersActivity.Companion.getAngleFinger6();
			...
			try {
				...
				for (i = 0; i<MAX_NUMBER_DETAILS; i++){
					...
					if (vbo[0] > 0 && ibo[0] > 0) {
						...
						errorHandler.handleError(com.bailout.stick.new_electronic_by_Rodeon.ui.activities.gripper.test_encoders.ErrorHandler.ErrorType.BUFFER_CREATION_ERROR, "glGenBuffers");
					}
				}
			} catch (Throwable t) {
				Timber.tag(TAG).w(t);
				errorHandler.handleError(com.bailout.stick.new_electronic_by_Rodeon.ui.activities.gripper.test_encoders.ErrorHandler.ErrorType.BUFFER_CREATION_ERROR, t.getLocalizedMessage());
			}
		}
