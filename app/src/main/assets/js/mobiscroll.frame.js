    ...
    ms.classes.Frame = function (el, settings, inherit) {
        ...
        that.position = function (check) {
            ...
            if (!that._isFullScreen && /modal|bubble/.test(s.display)) {
                ...
                //$wrapper.width(w + 3).css('white-space', totalw > nw ? '' : 'nowrap');
            }
            ...
            if (s.display == 'modal') {
                ...
                //l = constrain(al - ($popup.outerWidth(false) - aw) / 2, sl + 3, sl + nw - modalWidth - 3);
        ...
        that.show = function (prevAnim, prevFocus) {
            ...
            //тут генерируется разметка для разных типов отображения и тут же привязываются css классы
            // Create wheels containers
            html = '<div lang="' + s.lang + '" class="mbsc-' + s.theme + (s.baseTheme ? ' mbsc-' + s.baseTheme : '') + ' dw-' + s.display + ' ' +
                (s.cssClass || '') + '">' +
                '<div class="dw-persp">' +
                '<div' + ' class="dw' + ' dw-ltr' + '">' + // Popup
            ...
            if (hasButtons) {
                ...
                $.each(buttons, function (i, b) {
                    ...
                    html += '<div' + (s.btnWidth ? ' style="width:' + (200 / buttons.length) + '%"' : '') + ' class="dwbw ' + (b.parentClass || '') + '"><div tabindex="0" role="button" class="dwb' + i + ' dwb-e ' + (b.cssClass === undefined ? s.btnClass : b.cssClass) + (b.icon ? ' mbsc-ic mbsc-ic-' + b.icon : '') + '">' + (b.text || '') + '</div></div>';
                });
                html += '</div>';
            }
