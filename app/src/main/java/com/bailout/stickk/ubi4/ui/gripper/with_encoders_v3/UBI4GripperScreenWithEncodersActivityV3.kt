...
class UBI4GripperScreenWithEncodersActivityV3
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //control side in 3D
        side = if (useV3GestureProtocol) {
            gestureEditorViewModel.onAction(V3GestureEditorAction.ViewCreated(gestureID))
            gestureEditorViewModel.uiState.value.handSide
        } else {
            readUbi4HandSide()
        }
...
//                                + parameters.numberFinger + "     fingerAngel = " + parameters.fingerAngel)
                ...
                if (useV3GestureProtocol) {
                    // Animation scores belong to the UI; editable targets belong to the ViewModel.
                    when (numberFinger) {
                        1 -> score1 = angleFinger
                        2 -> score2 = angleFinger
                        3 -> score3 = angleFinger
                        4 -> score4 = angleFinger
                        5 -> score5 = validationRange(angleFinger)
                        6 -> score6 = validationRange(angleFinger)
                    }
                    gestureEditorViewModel.onAction(V3GestureEditorAction.FingerPositionChanged(numberFinger, angleFinger))
                    renderV3GestureTargets(gestureEditorViewModel.uiState.value)
                    return@subscribe
                }
                if (numberFinger == 1) {
...
//                    compileBLEMassage ()
        ...
        RxView.clicks(findViewById(R.id.gripperSaveBtn))
            ...
            .subscribe {
                if (useV3GestureProtocol) {
                    gestureEditorViewModel.onAction(V3GestureEditorAction.SaveClicked)
                    return@subscribe
                }
        ...
        lifecycleScope.launch(start = CoroutineStart.UNDISPATCHED) {
            viewModel.uiState.collect { state ->
                ...
                state.pendingSettings.forEach { update ->
                    ...
                    renderV3GestureTargets(viewModel.uiState.value)
                }
                if (state.closeRequested && !isFinishing) finish()
    ...
    private fun renderV3GestureTargets(state: V3GestureEditorUiState) {
        val settings = state.settings
        gestureState = state.command.code
        fingerOpenState1 = settings.openPositions[3]
        fingerOpenState2 = settings.openPositions[2]
        fingerOpenState3 = settings.openPositions[1]
        fingerOpenState4 = settings.openPositions[0]
        fingerOpenState5 = settings.openPositions[4]
        fingerOpenState6 = settings.openPositions[5]
        fingerCloseState1 = settings.closePositions[3]
        fingerCloseState2 = settings.closePositions[2]
        fingerCloseState3 = settings.closePositions[1]
        fingerCloseState4 = settings.closePositions[0]
        fingerCloseState5 = settings.closePositions[4]
        fingerCloseState6 = settings.closePositions[5]
        fingerOpenStateDelay1 = settings.openToCloseDelays[0]
        fingerOpenStateDelay2 = settings.openToCloseDelays[1]
        fingerOpenStateDelay3 = settings.openToCloseDelays[2]
        fingerOpenStateDelay4 = settings.openToCloseDelays[3]
        fingerOpenStateDelay5 = settings.openToCloseDelays[4]
        fingerOpenStateDelay6 = settings.openToCloseDelays[5]
        fingerCloseStateDelay1 = settings.closeToOpenDelays[0]
        fingerCloseStateDelay2 = settings.closeToOpenDelays[1]
        fingerCloseStateDelay3 = settings.closeToOpenDelays[2]
        fingerCloseStateDelay4 = settings.closeToOpenDelays[3]
        fingerCloseStateDelay5 = settings.closeToOpenDelays[4]
        fingerCloseStateDelay6 = settings.closeToOpenDelays[5]
    }
    ...
    private fun readUbi4HandSide(): Int {
    ...
    private fun startFingerTransition(targetState: FingerTargetState, sendBleCommand: Boolean) {
        ...
        if (useV3GestureProtocol) {
            if (sendBleCommand) gestureEditorViewModel.onAction(V3GestureEditorAction.PoseTransitionStarted(opening))
        } else {
            if (sendBleCommand) {
...
//        myDialog.findViewById<LottieAnimationView>(R.id.delay_fingers_animation_view).setAnimation(R.raw.loader_calibrating)
        ...
        myDialog.findViewById<SeekBar>(R.id.dialogFingersDelay1Sb).setOnSeekBarChangeListener(object : OnSeekBarChangeListener {
            ...
            override fun onStopTrackingTouch(seekBar: SeekBar?) {
                if (seekBar != null) {
                    if (useV3GestureProtocol) {
                        gestureEditorViewModel.onAction(V3GestureEditorAction.FingerDelayChanged(1, seekBar.progress))
                        return
                    }
                    if (gestureState == States.GESTURE_STATE_CLOSE.number) {
        ...
        myDialog.findViewById<SeekBar>(R.id.dialogFingersDelay2Sb).setOnSeekBarChangeListener(object : OnSeekBarChangeListener {
            ...
            override fun onStopTrackingTouch(seekBar: SeekBar?) {
                if (seekBar != null) {
                    if (useV3GestureProtocol) {
                        gestureEditorViewModel.onAction(V3GestureEditorAction.FingerDelayChanged(2, seekBar.progress))
                        return
                    }
                    if (gestureState == States.GESTURE_STATE_CLOSE.number) {
        ...
        myDialog.findViewById<SeekBar>(R.id.dialogFingersDelay3Sb).setOnSeekBarChangeListener(object : OnSeekBarChangeListener {
            ...
            override fun onStopTrackingTouch(seekBar: SeekBar?) {
                if (seekBar != null) {
                    if (useV3GestureProtocol) {
                        gestureEditorViewModel.onAction(V3GestureEditorAction.FingerDelayChanged(3, seekBar.progress))
                        return
                    }
                    if (gestureState == States.GESTURE_STATE_CLOSE.number) {
        ...
        myDialog.findViewById<SeekBar>(R.id.dialogFingersDelay4Sb).setOnSeekBarChangeListener(object : OnSeekBarChangeListener {
            ...
            override fun onStopTrackingTouch(seekBar: SeekBar?) {
                if (seekBar != null) {
                    if (useV3GestureProtocol) {
                        gestureEditorViewModel.onAction(V3GestureEditorAction.FingerDelayChanged(4, seekBar.progress))
                        return
                    }
                    if (gestureState == States.GESTURE_STATE_CLOSE.number) {
        ...
        myDialog.findViewById<SeekBar>(R.id.dialogFingersDelay5Sb).setOnSeekBarChangeListener(object : OnSeekBarChangeListener {
            ...
            override fun onStopTrackingTouch(seekBar: SeekBar?) {
                if (seekBar != null) {
                    if (useV3GestureProtocol) {
                        gestureEditorViewModel.onAction(V3GestureEditorAction.FingerDelayChanged(5, seekBar.progress))
                        return
                    }
                    if (gestureState == States.GESTURE_STATE_CLOSE.number) {
        ...
        myDialog.findViewById<SeekBar>(R.id.dialogFingersDelay6Sb).setOnSeekBarChangeListener(object : OnSeekBarChangeListener {
            ...
            override fun onStopTrackingTouch(seekBar: SeekBar?) {
                if (seekBar != null) {
                    if (useV3GestureProtocol) {
                        gestureEditorViewModel.onAction(V3GestureEditorAction.FingerDelayChanged(6, seekBar.progress))
                        return
                    }
                    if (gestureState == States.GESTURE_STATE_CLOSE.number) {
    ...
    private fun compileBLEMassage () {
        ...
        val command = BLECommands.sendGestureInfo(gestureStateModel)
        Log.i(
            FLOW_TAG,
