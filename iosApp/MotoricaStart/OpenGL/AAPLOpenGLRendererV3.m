...
//        NSLog(@"Линкуем в программу фрагментный и вершинный шейдеры");
        ...
        _templeProgram = [AAPLOpenGLRendererV3 buildProgramWithVertexSourceURL:vertexSourceURL
...
- (void) createSelectedProgram {
    ...
    _selectProgram = [AAPLOpenGLRendererV3 buildProgramWithVertexSourceURL:vertexSelectSourceURL
...
//    NSLog(@"Вызываем функцию обновления состояния кадра");
    ...
    const matrix_float4x4 startRotationMatrix_z   = matrix4x4_rotation (-_startAngleZ, templeRotationAxis_z);
    _startAngle = 0.0f;
    _startAngleZ = 0.0f;
    matrix_float4x4 templeRotationMatrix_x   = matrix4x4_identity();
    matrix_float4x4 templeRotationMatrix_y   = matrix4x4_identity();
    matrix_float4x4 templeRotationMatrix_z   = matrix4x4_identity();
    ...
    currentRotation                          = matrix_multiply(startRotationMatrix_z, currentRotation);
...
- (void) resize:(CGSize)size {
    ...
    if (_handSide == 1) {
        _projectionMatrix = matrix_perspective_right_hand_gl_v3(65.0f * (M_PI / 180.0f), aspect, 1.0f, 5000.0);
    } else {
        _projectionMatrix = matrix_perspective_left_hand_gl_v3(65.0f * (M_PI / 180.0f), aspect, 1.0f, 5000.0);
    }
...
matrix_float4x4 matrix_perspective_left_hand_gl_v3(float fovyRadians, float aspect, float nearZ, float farZ) {
...
matrix_float4x4 matrix_perspective_right_hand_gl_v3(float fovyRadians, float aspect, float nearZ, float farZ) {
    float ys = 1 / tanf(fovyRadians * 0.5);
    float xs = ys / aspect;
    float zs = (farZ + nearZ) / (farZ - nearZ);
    float ws = -(2.f * farZ * nearZ) / (farZ - nearZ);
...
- (void) endTouchIvent {
    ...
    if (_angleLittleFingerTransferOld != _angleLittleFingerTransfer) {
        ...
        if (stateGesture == 0) {
            ...
            _gestureWithAddress.gesture.openPosition4 = (int32_t)openStage1;
            ...
            _gestureWithAddress.gesture.closePosition4 = (int32_t)closeStage1;
    ...
    if (_angleRingFingerTransferOld != _angleRingFingerTransfer) {
        ...
        if (stateGesture == 0) {
            ...
            _gestureWithAddress.gesture.openPosition3 = (int32_t)openStage2;
            ...
            _gestureWithAddress.gesture.closePosition3 = (int32_t)closeStage2;
    ...
    if (_angleMiddleFingerTransferOld != _angleMiddleFingerTransfer) {
        ...
        if (stateGesture == 0) {
            ...
            _gestureWithAddress.gesture.openPosition2 = (int32_t)openStage3;
            ...
            _gestureWithAddress.gesture.closePosition2 = (int32_t)closeStage3;
    ...
    if (_angleForeFingerTransferOld != _angleForeFingerTransfer) {
        ...
        if (stateGesture == 0) {
            ...
            _gestureWithAddress.gesture.openPosition1 = (int32_t)openStage4;
            ...
            _gestureWithAddress.gesture.closePosition1 = (int32_t)closeStage4;
