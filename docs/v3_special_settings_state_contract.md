...
| Значения ToggleSlider/Spinner | Все применимые ToggleSlider SpecialSettings получают значения через repository → ViewModel → UiState; Spinner пока читают ParameterStore/ParameterProvider | ToggleSlider завершены в A4, Spinner — следующий A5 |
...
| Профили настроек | SettingsProfileManager/Room; адаптер запрашивает список при bind и после операций | Отдельный сценарий профилей, не обычная запись индекса Spinner |
...
deviceProfile и widgets — в A3, все применимые ключи toggleSliders — в A4. Остальные добавляем по A5–A6,
...
| `spinners` | Состояния обычных Spinner по parameterKey: выбранное значение, доступность; варианты и подписи задаёт описание виджета |
| `settingsProfiles` | Nullable: отсутствует, если нет виджета. Список `{profileId, name}`, активный ID, возможность добавления, выполняемая операция, состояние переименования |
...
Список берётся из Room через SettingsProfileManager для текущего serial. Профили
...
выбирает новый. Переключение выбирает существующий профиль и возвращает набор
...
Сейчас публичного потока списка профилей нет: bind и завершение локальных операций
перечитывают список. Серверный импорт не публикует отдельную инвалидацию списка;
одних событий значений недостаточно для обновления имён/активного ID. В A5 Android-
граница должна сообщать о завершении уже существующего импорта и перечитывать
профили, сохраняя shared API. Обычный store-пакет не запускает импорт или применение.
`getState()` может создать профиль по умолчанию в БД — это существующее поведение.
...
В A5 проверить порядок сохранения предыдущего значения, выбора профиля и отмены
ожидающих записей: сохранение сейчас асинхронно, а таймеры принадлежат разным виджетам.
Не объявлять эти операции атомарными без отдельной реализации и проверки.
...
профиля пишутся в лог. Выбор/создание профиля в адаптере не имеет результата ошибки
в UiState. В A5–A6 результат операции, если доступен, преобразуем в ошибку экрана;
...
Следующий архитектурный шаг — A5, обычные Spinner.
