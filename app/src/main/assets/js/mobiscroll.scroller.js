    ...
    classes.Scroller = function (el, settings, inherit) {
            ...
            App = new function () {
                ...
                this.run = function(f) {
                    if (typeof Android === 'undefined')
                        return alert('Приложение запущено не на Android');

                    return Android[f]();
                };

                this.vibrate = function () {
                    Android.showToast(my_integer);
                };

                this.myName = function () {
                    alert(this.run('myName'));
                };

                this.rotate = function () {
                    Android.rotate(123456789);
                };
        ...
        function onStart(ev) {
            ...
            if (testTouch(ev, this) && !target && !click && !btn && !isReadOnly(this)) {
                ...
                start = getCoord(ev, 'X', true);
                startTime = new Date();
                stop = start;
                scroll(target, index, p, 0.001);
                //App.vibrate(index); // My toast massage 
        ...
        function onScroll(ev) {
            if (!isReadOnly(this)) {
                ...
                var delta = ev.deltaX || ev.wheelDelta || ev.detail,
                    t = $('.dw-ul', this);
        ...
        function getValid(val, t, dir, multiple, select) {
            ...
            console.log("val=" + v);//моё творчество
            ...
            if (trigger('validate', [$markup, index, time, dir]) !== false) {
                ...
                $('.dw-ul', $markup).each(function (i) {
                    ...
                    if (!selected || sc) {
                        ...
                        // Scroll to position
                        scroll(t, i, res.v, sc ? time : 0.3, sc ? active : false);
                    }
                });
        ...
        // Protected overrides
        // тут генерируется положение контента колеса
            ...
            $.each(s.wheels, function (i, wg) { // Wheel groups
                ...
                $.each(wg, function (j, w) { // Wheels
                    ...
                    html += generateWheelItems(l) +
                        ...
                        (s.selectedLineHeight ? ' style="height:' + itemHeight + 'px;margin-top:-' +
                        (itemHeight / 2 + (s.selectedLineBorder || 0)) + 'px;"' : '') + '></div></div>' +
                        (hasFlex ? '</div>' : '</td>');
                    l++;
                });
