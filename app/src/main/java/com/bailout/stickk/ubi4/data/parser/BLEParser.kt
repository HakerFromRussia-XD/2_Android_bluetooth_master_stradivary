    ...
    internal fun parseReceivedData(data: ByteArray?) {
        if (data != null) {
            ...
            if (requestType == 1) {
                ...
                Log.d(
                    "TestOptic",
                    " parameter: ${ParameterProvider.getParameter(deviceAddress, parameterID)}"
                )
                updateAllUI(
                    deviceAddress,
                    parameterID,
                ...
                when (codeRequest) {
                    (0x00).toByte() -> {
                        System.err.println("TEST parser DEFOULT")
                    }
                    ...
                    BaseCommands.WRITE_FW_COMMAND.number -> {
                        System.err.println("TEST parser WRITE_FW_COMMAND")
                    }

                    BaseCommands.DEVICE_ACCESS_COMMAND.number -> {
                        System.err.println("TEST parser DEVICE_ACCESS_COMMAND")
                    }

                    BaseCommands.ECHO_COMMAND.number -> {
                        System.err.println("TEST parser ECHO_COMMAND")
                    }

                    BaseCommands.SUB_DEVICE_MANAGER.number -> {
                        System.err.println("TEST parser SUB_DEVICE_MANAGER")
                    }

                    BaseCommands.GET_DEVICE_STATUS.number -> {
                        System.err.println("TEST parser GET_DEVICE_STATUS")
                    }

                    BaseCommands.DATA_TRANSFER_SETTINGS.number -> {
                        System.err.println("TEST parser DATA_TRANSFER_SETTINGS")
                    }
...
//                )
    ...
    // All data parsers
    private fun parseDeviceInformation(
        packageCodeRequest: Byte,
        ID: Int,
        receiveDataString: String
    ) {
        when (packageCodeRequest) {
            (0x00).toByte() -> {
                System.err.println("TEST parser 2 DEFOULT")
            }
            ...
            DeviceInformationCommand.READ_SUB_DEVICES_FIRST_INFO.number -> {
                System.err.println("TEST parser 2 READ_SUB_DEVICES_FIRST_INFO")
            }
            ...
            DeviceInformationCommand.SUB_DEVICE_PARAMETER_INIT_READ.number -> {
                System.err.println("TEST parser 2 SUB_DEVICE_PARAMETER_INIT_READ")
            }

            DeviceInformationCommand.SUB_DEVICE_PARAMETER_INIT_WRITE.number -> {
                System.err.println("TEST parser 2 SUB_DEVICE_PARAMETER_INIT_WRITE")
            }

            DeviceInformationCommand.GET_SERIAL_NUMBER.number -> {
                System.err.println("TEST parser 2 GET_SERIAL_NUMBER")
            }

            DeviceInformationCommand.SET_SERIAL_NUMBER.number -> {
                System.err.println("TEST parser 2 SET_SERIAL_NUMBER")
            }

            DeviceInformationCommand.GET_DEVICE_NAME.number -> {
                System.err.println("TEST parser 2 GET_DEVICE_NAME")
            }

            DeviceInformationCommand.SET_DEVICE_NAME.number -> {
                System.err.println("TEST parser 2 SET_DEVICE_NAME")
            }

            DeviceInformationCommand.GET_DEVICE_ROLE.number -> {
                System.err.println("TEST parser 2 GET_DEVICE_ROLE")
            }

            DeviceInformationCommand.SET_DEVICE_ROLE.number -> {
                System.err.println("TEST parser 2 SET_DEVICE_ROLE")
            }
        }
    }
    ...
    private fun parseDataManger(packageCodeRequest: Byte, receiveDataString: String) {
        when (packageCodeRequest) {
            ...
            DataManagerCommand.READ_AVAILABLE_SLOTS.number -> {
                System.err.println("TEST parser 2 READ_AVAILABLE_SLOTS")
            }

            DataManagerCommand.WRITE_SLOT.number -> {
                System.err.println("TEST parser 2 WRITE_SLOT")
            }
            ...
            DataManagerCommand.WRITE_DATA.number -> {
                System.err.println("TEST parser 2 WRITE_DATA")
            }

            DataManagerCommand.RESET_TO_FACTORY.number -> {
                System.err.println("TEST parser 2 RESET_TO_FACTORY")
            }

            DataManagerCommand.SAVE_DATA.number -> {
                System.err.println("TEST parser 2 SAVE_DATA")
            }
        }
    }
    ...
    private fun parseInitializeInformation(receiveDataString: String) {
        fullInicializeConnectionStruct = Json.decodeFromString<FullInicializeConnectionStruct>(
            "\"${
                receiveDataString.substring(
                    18,
        ...
        for (i in 0 until fullInicializeConnectionStruct.parametrsNum) {
            listA.add(
                Json.decodeFromString<BaseParameterInfoStruct>(
                    "\"${
                        receiveDataString.substring(
                            20 + i * BASE_PARAMETER_INFO_STRUCT_SIZE,
                            20 + (i + 1) * BASE_PARAMETER_INFO_STRUCT_SIZE
                        )
                    }\""
                )
            )
        ...
        if (baseParametrInfoStructArray[ID].additionalInfoSize != 0) {
            for (i in 0 until baseParametrInfoStructArray[ID].additionalInfoSize) {
                ...
                val additionalInfoSizeStruct = Json.decodeFromString<AdditionalInfoSizeStruct>(
                    "\"${
                        receiveDataString.substring(
                            offset + i * ADDITIONAL_INFO_SIZE_STRUCT_SIZE,
                            offset + (i + 1) * ADDITIONAL_INFO_SIZE_STRUCT_SIZE
                        )
                    }\""
                )
                ...
                ) // оступ на кол-во байт в считываемом сегменте
//                                        System.err.println("testSignal 0 $receiveDataStringForParse")
                ...
                when (additionalInfoSizeStruct.infoType) {
                    AdditionalParameterInfoType.WIDGET.number -> {
                        parseWidgets(
                            receiveDataStringForParse,
                            parameterID = ID,
                            dataCode = baseParametrInfoStructArray[ID].dataCode
                        )
                        GlobalScope.launch {
...
//        Log.d("SubDeviceSubDevice", "receiveDataString=$receiveDataString")
        ...
        ) // 8 байт заголовок и отправленные данные
        ...
        Log.d(
            "SubDeviceSubDevice",
            "subDevices=$baseSubDevicesInfoStructSet  numerSubDevice=$numberSubDevice"
        )
        mMain.bleCommand(
    ...
    private fun parseReadSubDeviceParameters(receiveDataString: String) {
        ...
        for (i in 0 until baseSubDevicesInfoStructSet.elementAt(subDeviceCounter).parametrsNum) {
            ...
            Log.d("receiveDataString", "${receiveDataString.length}")
            if (end <= receiveDataString.length) {
                try {
                    ...
                    Log.e("parseReadSubDeviceParameters", "Ошибка при парсинге параметра: $e")
                }
            } else {
                Log.e("parseReadSubDeviceParameters", "Индексы $start-$end выходят за пределы строки длиной ${receiveDataString.length}")
                break
            }
//            listA.add(
//                Json.decodeFromString<BaseParameterInfoStruct>(
//                    "\"${
//                        receiveDataString.substring(
//                            22 + i * BASE_PARAMETER_INFO_STRUCT_SIZE,
//                            22 + (i + 1) * BASE_PARAMETER_INFO_STRUCT_SIZE
//                        )
//                    }\""
//                )
//            )
        ...
        if (getNextSubDevice(subDeviceCounter) != 0) {
            ...
            Log.d(
                "SubDeviceAdditionalParameters",
                "6 = ${getSubDeviceParameterWithAdditionalParameters(1).first}  0 = ${
                    getSubDeviceParameterWithAdditionalParameters(1).second
                }"
            )
    ...
    private fun parseReadSubDeviceAdditionalParameters(
        addressSubDevice: Int,
        parameterID: Int,
        receiveDataString: String
    ) {
            ...
            subDevice.parametersList.forEach { parametrSubDevice ->
                if (subDevice.deviceAddress == addressSubDevice) {
                    if (parametrSubDevice.ID == parameterID) {
                        Log.d(
                            "parseReadSubDeviceAdditionalParameters",
                            "deviceAddress=${subDevice.deviceAddress}   additionalInfoSize=${parametrSubDevice.additionalInfoSize}"
                        )
                        for (i in 0 until parametrSubDevice.additionalInfoSize) {
                            //каждый новый цикл вычитываем данные следующего сегмента (следующий addInfoSeg)
                            val additionalInfoSizeStruct =
                                Json.decodeFromString<AdditionalInfoSizeStruct>(
                                    "\"${
                                        receiveDataString.substring(
                                            offset + i * ADDITIONAL_INFO_SIZE_STRUCT_SIZE,
                                            offset + (i + 1) * ADDITIONAL_INFO_SIZE_STRUCT_SIZE
...
//                                        System.err.println("testSignal 0 $receiveDataStringForParse")
                            ...
                            Log.d(
                                "parseReadSubDeviceAdditionalParameters",
                                "receiveDataStringForParse = $receiveDataStringForParse"
                            )
                            ...
                            when (additionalInfoSizeStruct.infoType) {
                                AdditionalParameterInfoType.WIDGET.number.toInt() -> {
                                    parseWidgets(
                                        receiveDataStringForParse,
                                        parameterID = parametrSubDevice.ID,
                                        dataCode = parametrSubDevice.dataCode
                                    )
                                    GlobalScope.launch {
                                        mMain.sendWidgetsArray()
                                    }
    ...
    private fun getNextSubDeviceParameter(subDeviceParameterCounter: Int): Int {
        ...
        Log.d(
            "SubDeviceSubDevice",
            "baseSubDevicesInfoStructArray=${baseSubDevicesInfoStructSet.elementAt(subDeviceCounter).parametersList.size}"
        )
//        baseSubDevicesInfoStructArray[subDeviceCounter].parametrsList
    ...
    private fun areEqualExcludingSetIdS(
        obj1: BaseParameterWidgetSStruct,
        obj2: BaseParameterWidgetSStruct
    ): Boolean {
        // Сравниваем объекты, исключив поле parametersIDAndDataCodes
        val baseParameterWidgetStruct1 =
            obj1.baseParameterWidgetStruct.copy(parametersIDAndDataCodes = obj2.baseParameterWidgetStruct.parametersIDAndDataCodes)
    ...
    private fun areEqualExcludingSetIdE(
        obj1: BaseParameterWidgetEStruct,
        obj2: BaseParameterWidgetEStruct
    ): Boolean {
        // Сравниваем объекты, исключив поле parametersIDAndDataCodes
        val baseParameterWidgetStruct1 =
            obj1.baseParameterWidgetStruct.copy(parametersIDAndDataCodes = obj2.baseParameterWidgetStruct.parametersIDAndDataCodes)
    ...
    private fun parseWidgets(receiveDataStringForParse: String, parameterID: Int, dataCode: Int) {
        var baseParameterWidgetStruct =
            Json.decodeFromString<BaseParameterWidgetStruct>("\"${receiveDataStringForParse}\"")//+parameterID.toString()
            ...
            ParameterWidgetLabelType.PWLTE_CODE_LABEL.number.toInt() -> {
                when (baseParameterWidgetStruct.widgetCode) {
                    ParameterWidgetCode.PWCE_UNKNOW.number.toInt() -> {
                        System.err.println("parseWidgets UNKNOW")
                    }

                    ParameterWidgetCode.PWCE_BUTTON.number.toInt() -> {
                        val commandParameterWidgetEStruct =
                            Json.decodeFromString<CommandParameterWidgetEStruct>("\"${receiveDataStringForParse}\"")
                        commandParameterWidgetEStruct.baseParameterWidgetEStruct.baseParameterWidgetStruct.parametersIDAndDataCodes.add(
                    ...
                    ParameterWidgetCode.PWCE_SWITCH.number.toInt() -> {
                        ...
                        val switchParameterWidgetEStruct =
                            Json.decodeFromString<SwitchParameterWidgetEStruct>("\"${receiveDataStringForParse}\"")
                        switchParameterWidgetEStruct.baseParameterWidgetEStruct.baseParameterWidgetStruct.parametersIDAndDataCodes.add(
                            Pair(parameterID, dataCode)
                        )
                        listWidgets.add(switchParameterWidgetEStruct)
                    }

                    ParameterWidgetCode.PWCE_COMBOBOX.number.toInt() -> {
                        System.err.println("parseWidgets COMBOBOX")
                    }
                    ...
                    ParameterWidgetCode.PWCE_PLOT.number.toInt() -> {
                        ...
                        val plotParameterWidgetEStruct =
                            Json.decodeFromString<PlotParameterWidgetEStruct>("\"${receiveDataStringForParse}\"")
                        plotParameterWidgetEStruct.baseParameterWidgetEStruct.baseParameterWidgetStruct.parametersIDAndDataCodes.add(
                            Pair(parameterID, dataCode)
                        )
                    ...
                    ParameterWidgetCode.PWCE_EMG_GESTURE_CHANGE_SETTINGS.number.toInt() -> {
                        System.err.println("parseWidgets EMG_GESTURE_CHANGE_SETTINGS")
                    }

                    ParameterWidgetCode.PWCE_GESTURE_SETTINGS.number.toInt() -> {
                        System.err.println("parseWidgets GESTURE_SETTINGS")
                    }

                    ParameterWidgetCode.PWCE_CALIB_STATUS.number.toInt() -> {
                        System.err.println("parseWidgets CALIB_STATUS")
                    }

                    ParameterWidgetCode.PWCE_CONTROL_MODE.number.toInt() -> {
                        System.err.println("parseWidgets CONTROL_MODE")
                    }
                    ...
                    ParameterWidgetCode.PWCE_PLOT_AND_1_THRESHOLD.number.toInt() -> {
                        System.err.println("parseWidgets PLOT_AND_1_THRESHOLD")
                    }

                    ParameterWidgetCode.PWCE_PLOT_AND_2_THRESHOLD.number.toInt() -> {
                        System.err.println("parseWidgets PLOT_AND_2_THRESHOLD")
                    }

                    ParameterWidgetCode.PWCE_GESTURES_WINDOW.number.toInt() -> {
                        System.err.println("parseWidgets PWCE_GESTURES_WINDOW")
                        val gesturesParameterWidgetEStruct =
                            Json.decodeFromString<BaseParameterWidgetEStruct>("\"${receiveDataStringForParse}\"")
                        gesturesParameterWidgetEStruct.baseParameterWidgetStruct.parametersIDAndDataCodes.add(
                            Pair(parameterID, dataCode)
                        )
                        ...
                        listWidgets.forEach {
                            if (it is BaseParameterWidgetEStruct) {
                                if (areEqualExcludingSetIdE(gesturesParameterWidgetEStruct, it)) {
                                    canAdd = false
                                    it.baseParameterWidgetStruct.parametersIDAndDataCodes.add(
                                        Pair(
                                            parameterID,
                                            dataCode
                                        )
            ...
            ParameterWidgetLabelType.PWLTE_STRING_LABEL.number.toInt() -> {
                when (baseParameterWidgetStruct.widgetCode) {
                    ...
                    ParameterWidgetCode.PWCE_BUTTON.number.toInt() -> {
                        ...
                        val commandParameterWidgetSStruct =
                            Json.decodeFromString<CommandParameterWidgetSStruct>("\"${receiveDataStringForParse}\"")
                        commandParameterWidgetSStruct.baseParameterWidgetSStruct.baseParameterWidgetStruct.parametersIDAndDataCodes.add(
                            Pair(parameterID, dataCode)
                        )
                    ...
                    ParameterWidgetCode.PWCE_SWITCH.number.toInt() -> {
                        System.err.println("parseWidgets SWITCH")
                        val switchParameterWidgetSStruct =
                            Json.decodeFromString<SwitchParameterWidgetSStruct>("\"${receiveDataStringForParse}\"")
                        switchParameterWidgetSStruct.baseParameterWidgetSStruct.baseParameterWidgetStruct.parametersIDAndDataCodes.add(
                            Pair(parameterID, dataCode)
                        )
                        listWidgets.add(switchParameterWidgetSStruct)
                    }
                    ...
                    ParameterWidgetCode.PWCE_PLOT.number.toInt() -> {
                        ...
                        val plotParameterWidgetSStruct =
                            Json.decodeFromString<PlotParameterWidgetSStruct>("\"${receiveDataStringForParse}\"")
                        plotParameterWidgetSStruct.baseParameterWidgetSStruct.baseParameterWidgetStruct.parametersIDAndDataCodes.add(
                            Pair(parameterID, dataCode)
                        )
                    ...
                    ParameterWidgetCode.PWCE_GESTURES_WINDOW.number.toInt() -> {
                        System.err.println("parseWidgets PWCE_GESTURES_WINDOW")
                        val gesturesParameterWidgetSStruct =
                            Json.decodeFromString<BaseParameterWidgetSStruct>("\"${receiveDataStringForParse}\"")
                        gesturesParameterWidgetSStruct.baseParameterWidgetStruct.parametersIDAndDataCodes.add(
                            Pair(parameterID, dataCode)
                        )
                    ...
                    ParameterWidgetCode.PWCE_OPTIC_LERNING_WIDGET.number.toInt() -> {
                        System.err.println("parseWidgets PWCE_OPTIC_LERNING_WIDGET_STRING $receiveDataStringForParse")
                    }
                }
            }
        }
