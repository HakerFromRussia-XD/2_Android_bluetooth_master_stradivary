...
data class BaseParametrInfoStruct(
    val ID: Int,
    val broadcastID: Int,
    val dataCode: Int,
...
object BaseParametrInfoSerializer: KSerializer<BaseParametrInfoStruct> {
    override val descriptor: SerialDescriptor =
        PrimitiveSerialDescriptor("FullInicializeConnection", PrimitiveKind.STRING)

    override fun deserialize(decoder: Decoder): BaseParametrInfoStruct {
        ...
        if (string.length >= 30) {
            ...
            initRead = castUnsignedCharToInt(string.substring(16, 18).toInt(16).toByte()) shr 0 and 0b00000001
            initWrite = castUnsignedCharToInt(string.substring(16, 18).toInt(16).toByte()) shr 1 and 0b00000001
            synchType = castUnsignedCharToInt(string.substring(16, 18).toInt(16).toByte()) shr 2 and 0b00000111
            synchDirection = castUnsignedCharToInt(string.substring(16, 18).toInt(16).toByte()) shr 5 and 0b00000111
            synchPeriod = castUnsignedCharToInt(string.substring(18, 20).toInt(16).toByte())

            type = castUnsignedCharToInt(string.substring(20, 22).toInt(16).toByte())
            saveInMaster = castUnsignedCharToInt(string.substring(22, 24).toInt(16).toByte()) shr 0 and 0b00000001
            saveInSlave = castUnsignedCharToInt(string.substring(22, 24).toInt(16).toByte()) shr 1 and 0b00000001
            saveReserv = castUnsignedCharToInt(string.substring(22, 24).toInt(16).toByte()) shr 2 and 0b00111111

            additionalInfoSize = castUnsignedCharToInt(string.substring(24, 26).toInt(16).toByte())

            relatedParametrID = castUnsignedCharToInt(string.substring(26, 28).toInt(16).toByte())
            relatedDataCode = castUnsignedCharToInt(string.substring(28, 30).toInt(16).toByte())
        }

        return BaseParametrInfoStruct (
            ID = ID,
    ...
    override fun serialize(encoder: Encoder, value: BaseParametrInfoStruct) {
        val code: String =
            value.ID.toString()
