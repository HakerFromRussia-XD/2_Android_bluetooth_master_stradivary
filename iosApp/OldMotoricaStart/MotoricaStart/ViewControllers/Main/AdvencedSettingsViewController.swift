...
class AdvencedSettingsViewController: UIViewController {
    ...
    @objc func resultDialog(notification: Notification) {
        ...
        if (resultDialog == "writeSerialNumberAccept") {
            self.showToast(message:"жмакнули ок", font: .systemFont(ofSize: 12.0))
            print("пароль writeSerialNumberAccept")
    ...
    private func startWriteSerialNumber() {
        let data = Data(serialNumberTextField.text!.utf8)+Data([0x00])
        sendDataToINDY(dataForWrite: data, characteristic: sampleGattAttributes.SET_SERIAL_NUMBER)
        saveNewSerialNumber(newSerialNumber: serialNumberTextField.text!)
        
        //организовать тут смену серийника в верху приложения
        saveDataString(key: sampleGattAttributes.DEVICE_NAME, value: serialNumberTextField.text!)
        deviceName.text = serialNumberTextField.text!
    }
    ...
    private func initUI() {
        ...
        for item in savingParametrsMassString
        {
            ...
            if (item.key == SensorsViewController.sampleGattAttributes.SCALE) {
                ...
                case 0:
                    scaleText.text = NSLocalizedString("size_s_approval", comment: "")
                case 1:
                    scaleText.text = NSLocalizedString("size_m_approval", comment: "")
                case 2:
                    scaleText.text = NSLocalizedString("size_l_approval", comment: "")
                case 3:
                    scaleText.text = NSLocalizedString("size_xl_approval", comment: "")
                default:
                    scaleText.text = NSLocalizedString("size_s_approval", comment: "")
                }
            }
    ...
    private func saveNewSerialNumber(newSerialNumber: String) {
        loadDataString()
        for item in savingParametrsMassString {
            if (item.key == sampleGattAttributes.DEVICE_MAC) {
                print("MAC-addr in advansed settings: \(item.value)")
                saveDataString(key: item.value, value: newSerialNumber)
            }
        }
